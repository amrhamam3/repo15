...
class STLRenderer : GLSurfaceView.Renderer {
    ...
    fun setModel(model: STLModel) {
        ...
        val wireData = FloatArray(triCount * 6 * 3)
        ...
        wireframeBuffer = ByteBuffer.allocateDirect(wireData.size * 4).order(ByteOrder.nativeOrder())
            .asFloatBuffer().apply { put(wireData); position(0) }
