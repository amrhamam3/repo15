...
object STLParser {
    ...
    private const val MAX_FILE_SIZE = 500_000_000L // 500 MB limit
    private const val CHUNK_SIZE = 1_000_000 // Read 1MB chunks
    private const val MAX_VERTICES_IN_MEMORY = 100_000 // ~1.2 MB per array
    ...
    fun parse(context: Context, uri: Uri): STLModel {
        ...
        // Get file size first
        val fileSize = resolver.openInputStream(uri)?.use { it.available().toLong() }
        ...
        val headerBytes = ByteArray(minOf(512, fileSize.toInt()))
        ...
        return if (isAsciiSTL(headerBytes, fileSize)) {
            ...
            parseBinaryOptimized(context, uri, fileSize)
        }
    }
    ...
    private fun parseBinaryOptimized(context: Context, uri: Uri, fileSize: Long): STLModel {
        ...
        val verticesPerChunk = minOf(MAX_VERTICES_IN_MEMORY, triangleCount * 3)
    ...
    private fun parseAsciiStreaming(context: Context, uri: Uri): STLModel {
        val resolver = context.contentResolver
        val vertexList = ArrayList<Float>(MAX_VERTICES_IN_MEMORY * 3)
        val normalList = ArrayList<Float>(MAX_VERTICES_IN_MEMORY * 3)
                        ...
                        line.startsWith("vertex", ignoreCase = true) -> {
                            ...
                            if (parts.size >= 4) {
                                try {
                                    ...
                                    if (vertexList.size > MAX_VERTICES_IN_MEMORY * 3) {
                                        throw STLParseException("حجم الملف كبير جداً - لا يمكن معالجة الملف بالكامل")
                                    }
                                } catch (e: NumberFormatException) {
